...
export function WorkerFindGigs() {
  ...
  const handleClaim = useCallback((shiftId: string) => {
    ...
    // Validation: check shift is still open
          ...
          >
            ...
            <div className="mb-4">
              {selectedShift.status === 'open' && !claimedIds.has(selectedShift.id) ? (
                <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full border bg-success-50 text-success-700 border-success-200">
                  OPEN
                </span>
              ) : (
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedShift.requirements.map((req, i) => (
                    <span key={i} className="text-xs font-medium bg-surface-700 text-surface-300 px-2.5 py-1 rounded-lg border border-surface-600">
                      {req}
                    </span>
                  ))}
                </div>
